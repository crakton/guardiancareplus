...
export const batchInvoiceKeys = {
  ...
  list: (filters?: BatchInvoiceFilters) =>
    [...batchInvoiceKeys.lists(), filters] as const,
  ...
  historyList: (filters?: BatchHistoryFilters) =>
    [...batchInvoiceKeys.history(), filters] as const,
  config: () => [...batchInvoiceKeys.all, "config"] as const,
  stats: (filters?: BatchInvoiceFilters) =>
    [...batchInvoiceKeys.all, "stats", filters] as const,
  recentActivity: (limit?: number) =>
    [...batchInvoiceKeys.all, "recent", limit] as const,
  processingStatus: () =>
    [...batchInvoiceKeys.all, "processing-status"] as const,
  worker: (workerId: string) =>
    [...batchInvoiceKeys.all, "worker", workerId] as const,
  client: (clientId: string) =>
    [...batchInvoiceKeys.all, "client", clientId] as const,
};
...
export const useGetWorkerBatchInvoices = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      batchInvoiceService.getBatchInvoicesForWorker(workerId, filters),
    enabled: enabled && !!workerId,
    staleTime: 2 * 60 * 1000, // 2 minutes
  });
};

// Hook to get batch invoices for a specific client
export const useGetParticipantBatchInvoices = (
  ...
  return useQuery({
    queryKey: [...batchInvoiceKeys.client(clientId), filters],
    queryFn: () =>
      batchInvoiceService.getBatchInvoicesForParticipant(clientId, filters),
...
> => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: batchInvoiceKeys.recentActivity(),
      });
...
export const useQuickActions = () => {
  ...
  const sendToAll = useMutation({
    ...
    onSuccess: (_, batchInvoiceId) => {
      ...
      queryClient.invalidateQueries({ queryKey: batchInvoiceKeys.lists() });
      toast.success("Batch invoice sent to both client and worker");
    },
    onError: (error: Error) => {
  ...
  const sendToParticipant = useMutation({
    ...
    onSuccess: (_, batchInvoiceId) => {
      ...
      queryClient.invalidateQueries({ queryKey: batchInvoiceKeys.lists() });
      toast.success("Batch invoice sent to client");
    },
    onError: (error: Error) => {
      toast.error(`Failed to send batch invoice: ${error.message}`);
    },
  ...
  const generateCurrentWeek = useMutation({
    mutationFn: ({
      ...
      batchInvoiceService.quickActions.generateCurrentWeek(workerId, clientId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: batchInvoiceKeys.lists() });
      queryClient.invalidateQueries({ queryKey: batchInvoiceKeys.history() });
      queryClient.invalidateQueries({
  ...
  const generatePreviousWeek = useMutation({
    mutationFn: ({
      ...
      batchInvoiceService.quickActions.generatePreviousWeek(workerId, clientId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: batchInvoiceKeys.lists() });
      queryClient.invalidateQueries({ queryKey: batchInvoiceKeys.history() });
      queryClient.invalidateQueries({
  ...
  const generateCurrentMonth = useMutation({
    mutationFn: ({
      ...
      batchInvoiceService.quickActions.generateCurrentMonth(workerId, clientId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: batchInvoiceKeys.lists() });
      queryClient.invalidateQueries({ queryKey: batchInvoiceKeys.history() });
      queryClient.invalidateQueries({
...
export const useGetBatchInvoiceByNumber = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      batchInvoiceService.getBatchInvoiceByBatchNumber(batchNumber),

...
import { ClientInvitations } from "@/components/worker/ClientInvitations";
...
import {
  useGetWorkerOverview,
  useGetWorkerFinancial,
  useGetWorkerSchedule,
...
export default function WorkerDashboard() {
  ...
  const {
    ...
    error: overviewError,
  } = useGetWorkerOverview("month", true);
  const {
    data: financialData,
    isLoading: financialLoading,
    error: financialError,
  } = useGetWorkerFinancial("month");
  const {
    data: scheduleData,
    isLoading: scheduleLoading,
    error: scheduleError,
  } = useGetWorkerSchedule("month");
  const {
    data: performanceData,
    isLoading: performanceLoading,
    error: performanceError,
  } = useGetWorkerPerformance("month");

  // Format data for charts
  const earningsTrendData = financialData?.earnings?.trend || [];
  const weeklyHoursData = scheduleData?.weeklyHours || [];
  const shiftsByServiceType =
    scheduleData?.shiftDistribution?.byServiceType || {};
    ...
    <div className="container py-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <StatCard
          title="Earnings"
          value={`$${
            overviewData?.workSummary?.earnings?.current?.toFixed(2) || "0.00"
          }`}
          icon={<DollarSign size={24} className="text-guardian" />}
          change={{
            value: `${
              overviewData?.workSummary?.earnings?.percentageChange || 0
            }% from last month`,
        ...
        <StatCard
          ...
          value={
            overviewData?.performanceMetrics?.averageRating > 0
              ? `${overviewData.performanceMetrics.averageRating.toFixed(1)}/5`
              : "Not rated"
          }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {overviewData.workSummary.upcomingShifts
                      ...
                      .map((shift: any, index: number) => (
                        <Card
                          key={index}
                          className="border-guardian/10 transition-all duration-200 hover:shadow-md hover:border-guardian/20"
                        >
                          <CardContent className="p-5">
                            <div className="flex justify-between items-start mb-4">
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  handleViewShiftDetails(shift._id)
                                }
                              >
                                View Details
                              </Button>
        ...
        <Card className="border-guardian/10 transition-all duration-200 hover:shadow-lg">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(shiftsByServiceType).map(
                ([serviceType, count]) => (
                  <div
                    key={serviceType}
                  ...
                  >
                    ...
                    <Progress
                      value={
                        (count /
                          Math.max(...Object.values(shiftsByServiceType))) *
                        100
                      }
                      className="mt-2 h-2"
                    />
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                  ...
                  <span className="font-semibold text-green-600">
                    ${financialData.earnings.currentPeriod.toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
                  ...
                  <span className="font-semibold text-yellow-600">
                    ${financialData.earnings.pending.toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                  ...
                  <span className="font-semibold text-blue-600">
                    ${financialData.projections.averageHourlyRate.toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  ...
                  <span className="font-semibold text-gray-600">
                    ${financialData.expenses.travelReimbursements.toFixed(2)}
                  </span>
                </div>
              </div>
            </CardContent>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-guardian">
                ...
                <Progress
                  value={overviewData?.performanceMetrics?.completionRate || 0}
                  className="h-2"
                />
                ...
                <Progress
                  value={overviewData?.performanceMetrics?.onTimeRate || 0}
                  className="h-2"
                />
                  ...
                  <>
                    <div className="pt-2 border-t">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="font-semibold text-guardian">
                          {performanceData.availabilityComparison.utilizationPercentage.toFixed(
                            1
                          )}
                          %
                        </span>
                      ...
                      <div className="text-xs text-muted-foreground space-y-1">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span>
                            {performanceData.availabilityComparison.bookedHours}
                            h
                          </span>
                        </div>
                      </div>

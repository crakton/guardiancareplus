...
export default function AdminAnalyticsDashboard() {
  ...
  const generateOverviewMetrics = () => {
    ...
    return [
      ...
      },
      {
        ...
        value: formatNumber(
          overviewData?.platformActivity?.activeHouseholds ?? 0
        ),
  ...
  // Transform top households data for bar chart
  const getTopHouseholdsChart = () => {
    if (!financialData)
      return { data: [], dataKeys: ["revenue"], xAxisKey: "name" };

    return {
      data: financialData?.revenue?.byHousehold.slice(0, 5),
      dataKeys: ["revenue"],
      xAxisKey: "name",
    };
  };
      ...
      >
        ...
        <TabsContent value="financial" className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <ChartCard
              title="Top Households by Revenue"
              type="bar"
              data={getTopHouseholdsChart().data}
              dataKeys={getTopHouseholdsChart().dataKeys}
              xAxisKey={getTopHouseholdsChart().xAxisKey}
              colors={["#2195F2"]}
              loading={isLoadingFinancial}
              icon={<Building2 className="h-5 w-5 text-guardian" />}
              description="Highest revenue generating households"
            />
          </div>

...
import { organizationService } from "./HouseholdsPage";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function ChatsList() {
  ...
  const {
    ...
    queryKey: ["households"],
    queryFn: async () => {
      return await organizationService.getHouseholds();
    },
  });
  ...
  const handleNewDirectChat = async (userId: string) => {
    ...
    try {
      ...
      const newConversation = await createNewConversation(
        ...
        "Direct Chat",
        "Testing Direct Chat",
        households[0]?._id // Using first household for demo
      );
      ...
      if (newConversation) {
        ...
        if (user.role === "supportWorker") {
          ...
          navigate(`/client/chat/${newConversation._id}`);
        } else {
  ...
  if (loadingUsers || loadingHouseholds) {
    return <Loader />;
  }

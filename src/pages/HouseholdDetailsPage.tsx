...
const organizationDetailService = {
  getHousehold: async (id: string): Promise<Household> => {
    // For now, we'll use the households endpoint and filter
    // In a real app, this would be a dedicated endpoint like /households/:id
    const response = await get<{ households: Household[] }>("/households");
    const household = response.households.find((org) => org._id === id);
    if (!household) {
...
export default function HouseholdDetailsPage() {
  ...
  const {
    ...
    queryKey: ["household-details", id],
    queryFn: () => organizationDetailService.getHousehold(id!),
    enabled: !!id,
  });
  ...
  // Calculate stats
  const stats = household
    ? {
        totalWorkers: household.workers.length,
        totalInvites: household.pendingInvites.length,
        pendingInvites: household.pendingInvites.filter(
          (invite) => invite.status === "pending"
        ).length,
        declinedInvites: household.pendingInvites.filter(
          (invite) => invite.status === "declined"
        ).length,
        averageBaseRate:
          household.workers.length > 0
            ? Math.round(
                household.workers.reduce(
                  (sum, worker) => sum + worker.serviceAgreement.baseHourlyRate,
                  0
                ) / household.workers.length
              )
            : 0,
        averageDistanceRate:
          household.workers.length > 0
      ...
      <div className="p-6">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            There was an error loading the household details. Please try again.
          </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-medium">
                  {format(parseISO(household.createdAt), "MMMM dd, yyyy")}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-medium">
                  {format(parseISO(household.updatedAt), "MMMM dd, yyyy")}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-medium font-mono text-xs">{household._id}</p>
              </div>
            </div>
          </div>
        </CardContent>
            ...
            <CardHeader className="cursor-pointer hover:bg-gray-50 transition-colors">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <CardTitle className="text-xl">
                    Pending Invitations ({household.pendingInvites.length})
                  </CardTitle>
                </div>

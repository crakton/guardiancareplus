...
interface WorkerOnboardingResponse {
  success: boolean;
  message: string;
}
...
const authService = {
  ...
  completeWorkerOnboarding: async (
    data: WorkerOnboardingInput
  ): Promise<WorkerOnboardingResponse> => {
    return await post<WorkerOnboardingResponse>(
      "/users/workers/onboarding",
      data
    );
  },
};

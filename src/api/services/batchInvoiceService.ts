...
export const batchInvoiceService = {
  ...
  getBatchInvoices: async (
    filters: BatchInvoiceFilters = {}
  ): Promise<BatchInvoicesResponse> => {
    const queryString = buildQueryString(filters);
    const url = queryString
      ? `/batch-invoices?${queryString}`
      : "/batch-invoices";
  ...
  getBatchInvoiceById: async (
    batchInvoiceId: string
  ): Promise<BatchInvoice> => {
    const response = await get<BatchInvoiceResponse>(
      `/batch-invoices/${batchInvoiceId}`
    );
    return response.batchInvoice;
  },

  // Get batch processing history
  getBatchProcessingHistory: async (
    filters: BatchHistoryFilters = {}
  ): Promise<BatchProcessingHistoryResponse> => {
    const queryString = buildQueryString(filters);
    const url = queryString
      ? `/batch-invoices/history?${queryString}`
      : "/batch-invoices/history";
  ...
  updateBatchConfig: async (
    configUpdate: BatchConfigUpdateRequest
  ): Promise<BatchConfig> => {
  ...
  generateBatchInvoice: async (
    generationRequest: BatchGenerationRequest
  ): Promise<BatchProcessingHistory> => {
    const response = await post<
      BatchGenerationResponse,
      BatchGenerationRequest
    >("/batch-invoices/generate", generationRequest);
  ...
  downloadBatchInvoiceFile: async (
    batchInvoiceId: string,
    fileName?: string
  ): Promise<void> => {
    try {
      console.log("Starting download for batch invoice:", batchInvoiceId);
      const blob = await batchInvoiceService.downloadBatchInvoice(
        batchInvoiceId
      );
      ...
      if (
        ...
        try {
          ...
          throw new Error(
            errorResponse.message || "Failed to download invoice"
          );
        } catch (parseError) {
          throw new Error(
            "Failed to download invoice: Invalid response format"
          );
        }
      }
      ...
      const downloadFileName =
        fileName || `batch-invoice-${batchInvoiceId}.pdf`;
  ...
  getBatchInvoiceStats: async (
    ...
    const response = await batchInvoiceService.getBatchInvoices(
      allInvoicesFilters
    );
    ...
    const stats = {
      ...
      completedCount: invoices.filter((inv) => inv.status === "completed")
        .length,
      failedCount: invoices.filter((inv) => inv.status === "failed").length,
      cancelledCount: invoices.filter((inv) => inv.status === "cancelled")
        .length,
  ...
  getRecentBatchActivity: async (
    limit: number = 5
  ): Promise<BatchProcessingHistory[]> => {
    const response = await batchInvoiceService.getBatchProcessingHistory({
      limit,
    });
  ...
  getBatchInvoiceByBatchNumber: async (
    batchNumber: string
  ): Promise<BatchInvoice | null> => {
    try {
      // Since there's no specific endpoint for batch number search, we'll get all and filter
      const response = await batchInvoiceService.getBatchInvoices({
        limit: 1000,
      });
      const invoice = response.batchInvoices.find(
  ...
  getBatchInvoicesForWorker: async (
    workerId: string,
    filters: BatchInvoiceFilters = {}
  ): Promise<BatchInvoicesResponse> => {
    ...
    const filteredInvoices = response.batchInvoices.filter(
      (inv) => inv.workerId._id === workerId
    );
  ...
  // Get batch invoices for a specific client
  getBatchInvoicesForParticipant: async (
    clientId: string,
    filters: BatchInvoiceFilters = {}
  ): Promise<BatchInvoicesResponse> => {
    // Add client filter logic if the API supports it, otherwise filter client-side
    const response = await batchInvoiceService.getBatchInvoices(filters);

    // Filter by client ID client-side
    const filteredInvoices = response.batchInvoices.filter(
      (inv) => inv.clientId._id === clientId
    );
